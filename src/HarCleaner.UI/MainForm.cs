...
public partial class MainForm : Form
{
    ...
    private void LoadInputFile(string filePath)
    {
        ...
        var extension = cmbOutputType.SelectedItem?.ToString()?.ToLower() == "ml-ingest" ? "json" : "har";
    ...
    private async Task ProcessHarFileAsync()
    {
        await Task.Run(async () =>
        {
            ...
            // Save the result
            if (_filterOptions.OutputType?.ToLower() == "ml-ingest")
            {
            ...
            _outputContent = File.ReadAllText(_filterOptions.OutputFile);
    ...
    private void btnSaveOutput_Click(object sender, EventArgs e)
    {
        ...
        var extension = _filterOptions.OutputType?.ToLower() == "ml-ingest" ? "json" : "har";
        using var dialog = new SaveFileDialog
        {
            Filter = $"{extension.ToUpper()} Files (*.{extension})|*.{extension}|All Files (*.*)|*.*",
