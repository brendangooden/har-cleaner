...
public class PrivacyFilterTests
{
    ...
    public void ShouldInclude_RemovePersonalIdentifiers_RedactsQueryParameters()
    {
        ...
        // Assert
        Assert.True(result);
        var sessionParam = entry.Request.QueryString.FirstOrDefault(q => q.Name == "session_id");
        var tokenParam = entry.Request.QueryString.FirstOrDefault(q => q.Name == "user_token");
        var safeParam = entry.Request.QueryString.FirstOrDefault(q => q.Name == "safe_param");
    ...
    public void ShouldInclude_AllPrivacyOptionsEnabled_CleansAllSensitiveData()
    {
        ...
        var sessionParam = entry.Request.QueryString.FirstOrDefault(q => q.Name == "session");
