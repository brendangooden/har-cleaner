...
public class MlIngestExporterTests
{
	...
	public void ConvertToMlIngestFormat_ShouldIncludeRequestAndResponseBodies()
	{
		...
		// Act
		var result = exporter.ConvertToMlIngestFormat(harFile);
		...
		Assert.Contains("John Doe", entry.Request.Body, StringComparison.OrdinalIgnoreCase);
		Assert.Contains("created", entry.Response.Body, StringComparison.OrdinalIgnoreCase);
		Assert.Contains("sessionId=xyz789", entry.Request.Cookies, StringComparison.OrdinalIgnoreCase);
		Assert.Contains("page=1", entry.Request.QueryParams, StringComparison.OrdinalIgnoreCase);
	}
	...
	public void RequestType_ShouldDetectChromeDevToolsResourceTypes()
	{
		...
		var documentEntry = result.First(e => string.Equals(e.ResourceType, "document", StringComparison.OrdinalIgnoreCase));
		Assert.Equal("document", documentEntry.RequestType);

		// XHR should be "xhr" type
		var xhrEntry = result.First(e => string.Equals(e.ResourceType, "xhr", StringComparison.OrdinalIgnoreCase));
		Assert.Equal("xhr", xhrEntry.RequestType);

		// Stylesheet should be "stylesheet" type
		var stylesheetEntry = result.First(e => string.Equals(e.ResourceType, "stylesheet", StringComparison.OrdinalIgnoreCase));
		Assert.Equal("stylesheet", stylesheetEntry.RequestType);

		// Fetch should be "fetch" type
		var fetchEntry = result.First(e => string.Equals(e.ResourceType, "fetch", StringComparison.OrdinalIgnoreCase));
